...
|---|---|
|`print`|Prints a string to stdout without a newline. Minimal formatting support.|
|`println`|Prints a string to stdout **with a newline**. Still minimal formatting support.|
|`fmt.Printf`|Allows rich, **formatted strings** with placeholders like `%d`, `%s`, etc.|
|`fmt.Println`|Prints values separated by spaces and ends with a newline.|
|`fmt.Sprintf`|Like `Printf`, but returns the formatted string instead of printing it.|
